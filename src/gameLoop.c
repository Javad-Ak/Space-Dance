...
void loadBmp()
{
    ...
    BomberSurface = IMG_Load(BOMBER_PP);
    ...
    DroneSurface = IMG_Load(DRONE_PP);
    ...
    FighterSurface = IMG_Load(FIGHTER_PP);
    ...
    healthSurface = IMG_Load(HEALTH_PP);
    ...
    bulletUpSurface = IMG_Load(BULLET_UP_PP);
    ...
    explodeSurface = IMG_Load(EXPLODE_PP);
...
void renderGame()
{
    ...
    while (current != NULL)
    {
        ...
        case fighterT:
            ...
            if (current->attach)
                SDL_RenderFillRect(renderer, &current->attachRect);
            break;
        case shipT:
            if (current->attach == 1)
    ...
    {
        if (score >= ShownScore + 50)
            ShownScore = score;
        if (shownHealth != Jet.Health)
            shownHealth = Jet.Health;
        if (shownLevel != Jet.bulletLevel)
            shownLevel = Jet.bulletLevel;
...
void renderMovingBackground()
{
    ...
    static int backgroundY2 = -1 * HEIGHT;

    backgroundY1 += cameraSpeed;
    backgroundY2 += cameraSpeed;
    ...
    SDL_Rect background1 = ScreenRect;
    SDL_Rect background2 = ScreenRect;

    background1.y = backgroundY1;
    background2.y = backgroundY2;

    SDL_RenderCopy(renderer, GameBGT, NULL, &background1);
    SDL_RenderCopy(renderer, GameBGT, NULL, &background2);
}
    ...
    while (SDL_PollEvent(&event) != 0)
    {
        ...
        else if (event.type == SDL_KEYDOWN)
        {
            ...
            case SDLK_DOWN:
                ...
                break;
            case SDLK_SPACE:
                if (SDL_GetTicks() - Jet.lastShot < 100)
                    continue;
...
int isPositionValid(int tmpX, int width)
{
    ...
    while (current != NULL)
    {
        ...
        if (tmpX < width || tmpX > WIDTH - width)
            return FALSE;

        if (current->enemyRect.x >= tmpX)
        {
            if (current->enemyRect.x - tmpX < width)
                return FALSE;
        }
        else
        {
            if (tmpX - current->enemyRect.x < current->enemyRect.w)
                return FALSE;
        }
        current = current->next;
    }
...
void updateGame()
{
    ...
    while (current != NULL)
    {
        ...
        if (current->type != explodeT && current->health <= 0)
            dropExplode(current);
        ...
        if (Jet.Health > 10)
            Jet.Health = 10;
        if (Jet.bulletLevel > 3)
            Jet.bulletLevel = 3;
        if (Jet.bulletLevel < 1)
            Jet.bulletLevel = 0;
        if (Jet.bulletLevel <= 0)
            Jet.Health = 0;
...
void updateFighter(enemyStruct *current)
{
    ...
    else if (current->attach)
    {
        ...
        if (current->attachRect.y >= HEIGHT)
            current->attach = FALSE;
    }
}
...
{
    if (current->health > 0)
        current->enemyRect.y += cameraSpeed * 2;
...
void updateDrone(enemyStruct *current)
{
    ...
    if (current->enemyRect.y > Jet.JetRect.y + Jet.JetRect.h + 10)
        return;
...
void terminateGame()
{
    ...
    if (BomberTexture != NULL)
        SDL_DestroyTexture(BomberTexture);
    if (BomberSurface != NULL)
        SDL_FreeSurface(BomberSurface);
    ...
    if (FighterTexture != NULL)
        SDL_DestroyTexture(FighterTexture);
    if (FighterSurface != NULL)
    ...
    if (bulletUpTexture != NULL)
        SDL_DestroyTexture(bulletUpTexture);
    if (bulletUpSurface != NULL)
        SDL_FreeSurface(bulletUpSurface);

    if (healthTexture != NULL)
        SDL_DestroyTexture(healthTexture);
    if (healthSurface != NULL)
